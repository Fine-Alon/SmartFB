...
async def get_global_statistics() -> dict:
    try:
        ...
        active_forms = db.surveys.count_documents({"is_active": True})

        # Category breakdown for Pie Chart
        category_pipeline = [
            {"$group": {"_id": "$ai_analysis.category", "count": {"$sum": 1}}}
        ]
        category_cursor = db.submissions.aggregate(category_pipeline)
        category_distribution = []
        for doc in category_cursor:
        ...
        time_cursor = db.submissions.aggregate(time_pipeline)
        submissions_over_time = []
        for doc in time_cursor:
            if doc.get("_id"):
